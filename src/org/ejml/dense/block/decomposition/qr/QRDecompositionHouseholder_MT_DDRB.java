 ...
 * and W is computed using {@link BlockHouseHolder_MT_DDRB#computeW_Column}.
 *
 * <p>
...
public class QRDecompositionHouseholder_MT_DDRB
    ...
    public void applyQ( DMatrixRBlock B, boolean isIdentity ) {
        ...
        for (int i = start; i >= 0; i -= blockLength) {
            ...
            if (!saveW)
                BlockHouseHolder_MT_DDRB.computeW_Column(blockLength, Y, W, workspace, gammas, Y.col0);

            // Apply the Qi to Q
            BlockHouseHolder_MT_DDRB.multTransA_vecCol(blockLength, Y, subB, WTA);
            MatrixMult_MT_DDRB.multPlus(blockLength, W, WTA, subB);
        }
    }
...
//            W.original.reshape(W.row1,W.col1,false);
            ...
            // Apply the Qi to Q
            MatrixMult_MT_DDRB.multTransA(blockLength, W, subB, WTA);
            BlockHouseHolder_MT_DDRB.multAdd_zeros(blockLength, Y, WTA, subB);
        }
    }
    ...
    public boolean decompose( DMatrixRBlock orig ) {
        ...
        for (int j = 0; j < m; j += blockLength) {
            ...
            if (!BlockHouseHolder_MT_DDRB.decomposeQR_block_col(blockLength, Y, gammas)) {
                return false;
            }
    ...
    protected void updateA( DSubmatrixD1 A ) {
        ...
        if (A.col1 > A.col0) {
            ...
            MatrixMult_MT_DDRB.multTransA(blockLength, W, A, WTA);
            BlockHouseHolder_MT_DDRB.multAdd_zeros(blockLength, Y, WTA, A);
        } else if (saveW) {
            BlockHouseHolder_MT_DDRB.computeW_Column(blockLength, Y, W, workspace, gammas, Y.col0);
        }
    }
