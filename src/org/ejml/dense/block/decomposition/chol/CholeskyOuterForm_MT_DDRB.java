...
public class CholeskyOuterForm_MT_DDRB implements CholeskyDecomposition_F64<DMatrixRBlock> {
    ...
    private boolean decomposeLower() {
        ...
        for (int i = 0; i < T.numCols; i += blockLength) {
            ...
            if (widthA == blockLength) {
                // B = L^-1 B
                TriangularSolver_MT_DDRB.solveBlock(blockLength, false, subA, subB, false, true);

                // C = C - B * B^T
                InnerRankUpdate_MT_DDRB.symmRankNMinus_L(blockLength, subC, subB);
            }
        }
    ...
    private boolean decomposeUpper() {
        ...
        for (int i = 0; i < T.numCols; i += blockLength) {
            ...
            if (widthA == blockLength) {
                // B = U^-1 B
                TriangularSolver_MT_DDRB.solveBlock(blockLength, true, subA, subB, true, false);

                // C = C - B^T * B
                InnerRankUpdate_MT_DDRB.symmRankNMinus_U(blockLength, subC, subB);
            }
        }
