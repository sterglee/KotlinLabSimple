...
public class CholeskyOuterForm_MT_FDRB implements CholeskyDecomposition_F32<FMatrixRBlock> {
    ...
    private boolean decomposeLower() {
        ...
        for (int i = 0; i < T.numCols; i += blockLength) {
            ...
            if (widthA == blockLength) {
                // B = L^-1 B
                TriangularSolver_MT_FDRB.solveBlock(blockLength, false, subA, subB, false, true);

                // C = C - B * B^T
                InnerRankUpdate_MT_FDRB.symmRankNMinus_L(blockLength, subC, subB);
            }
        }
    ...
    private boolean decomposeUpper() {
        ...
        for (int i = 0; i < T.numCols; i += blockLength) {
            ...
            if (widthA == blockLength) {
                // B = U^-1 B
                TriangularSolver_MT_FDRB.solveBlock(blockLength, true, subA, subB, true, false);

                // C = C - B^T * B
                InnerRankUpdate_MT_FDRB.symmRankNMinus_U(blockLength, subC, subB);
            }
        }
