...
public class TridiagonalDecompositionHouseholder_MT_DDRB
    ...
    public DMatrixRBlock getQ( @Nullable DMatrixRBlock Q, boolean transposed ) {
        ...
        for (int i = start; i >= 0; i -= A.blockLength) {
            ...
            // F=U^T*Q(i)
            if (transposed)
                MatrixMult_MT_DDRB.multTransB(A.blockLength, subQ, subU, tmp);
            else
                MatrixMult_MT_DDRB.mult(A.blockLength, subU, subQ, tmp);
            // Q(i+1) = Q(i) + W*F
            if (transposed)
                MatrixMult_MT_DDRB.multPlus(A.blockLength, tmp, subW, subQ);
            else
                MatrixMult_MT_DDRB.multPlusTransA(A.blockLength, subW, tmp, subQ);
