...
import kotlinLabSci.FFT.FFTResults;
import kotlinLabSci.math.array.Matrix;
import kotlinLabSci.math.array.Vec;
import kotlinLabSci.math.plot.plotObjects.Axis;
import kotlinLabSci.math.plot.plotObjects.LatexImage;
import kotlinLabSci.math.plot.plotObjects.texLabel;
import kotlinLabSci.math.plot.render.AbstractDrawer;
import java.util.Vector;

import static kotlinLabSci.math.plot.utils.PArray.*;

// this file implements a MATLAB like interface for plotting in KotlinLab
public class plot  {

        static public boolean  scatterPlotOn = false;  // plot only points without connecting by lines
      ...
      // plots every point, can be slow for large arrays
...
// var x = vrand(500); plot(x);  markX(100, 200, new Font("Arial", Font.BOLD, 30));  markX(10.0, 0.3)
        static public void xlabel(String xLabelStr) {
            currentPlot.setAxisLabel(kotlinLabSci.math.plot.PlotPanel.xAxisId, xLabelStr);
        }

        static public void ylabel(String yLabelStr) {
            currentPlot.setAxisLabel(kotlinLabSci.math.plot.PlotPanel.yAxisId, yLabelStr);
        }

        static public void zlabel(String zLabelStr) {
            if (currentPlot instanceof  Plot3DPanel)
                currentPlot.setAxisLabel(kotlinLabSci.math.plot.PlotPanel.zAxisId, zLabelStr);
        }
        ...
        public static void  latexLabel(String latex)  {
            texLabel  txl = new kotlinLabSci.math.plot.plotObjects.texLabel(latex, 1, 1);
            currentPlot.addPlotable(txl);
        }
        
        public static void  latexLabel(String latex, int coordx, int coordy)  {
            texLabel  txl = new kotlinLabSci.math.plot.plotObjects.texLabel(latex, coordx, coordy);
            currentPlot.addPlotable(txl);
        }
        
        public static void  latexLabel(String latex, int size,  int coordx, int coordy)  {
            texLabel  txl = new kotlinLabSci.math.plot.plotObjects.texLabel(latex, size, coordx, coordy);
            currentPlot.addPlotable(txl);
        }
        ...
        public static void latexLabel3d(String latex, double nw1, double nw2, double nw3, double se1, double se2, double se3, double sw1, double sw2, double sw3) {
            LatexImage tximg = new kotlinLabSci.math.plot.plotObjects.LatexImage(latex, nw1,  nw2, nw3, se1, se2, se3, sw1, sw2, sw3);
            currentPlot.addPlotable(tximg);
        }
        
        public static void latexLabel3d(String latex, int xcoord, int ycoord) {
            LatexImage tximg = new kotlinLabSci.math.plot.plotObjects.LatexImage(latex, xcoord, ycoord);
            currentPlot.addPlotable(tximg);
        }
...
static public PlotPanel plotv(Vector <Double>  x, Vector <Double> y, Vector <Double> z, Color color, String name) {
  ...
  // Apache Commons plotting
    kotlinLabSci.math.plot.PlotPanel  plotApacheCommonsComplexArray(org.apache.commons.math3.complex.Complex  [] y)
             {
        ...
        return kotlinLabSci.math.plot.plot.plot(t, yreals,  color, plotType,  plotName, lineWidth);
      }


    // plot FFT results, the real values
  static public kotlinLabSci.math.plot.PlotPanel  plotfftReals(FFTResults  y)
             {
              Color color = Color.GREEN;
              String plotName = "Real Values";
              int lineWidth = 1;
        ...
        return kotlinLabSci.math.plot.plot.plot(t, y.realFFTs,  color, plotType,  plotName, lineWidth);
      }

  // plot FFT results, the imaginary values
static public   kotlinLabSci.math.plot.PlotPanel  plotfft(FFTResults  y)  {
     return plotfftReals(y);
  }
  
    // plot FFT results, the imaginary values
 static public  kotlinLabSci.math.plot.PlotPanel  plotfftImags(FFTResults  y)
             {
              Color color = Color.GREEN;
              String plotName = "Real Values";
              int lineWidth = 1;
        ...
        return kotlinLabSci.math.plot.plot.plot(t, y.imFFTs,  color, plotType,  plotName, lineWidth);
      }
